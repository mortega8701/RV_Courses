...
int DisplCount=0, delay = 0x10000;
...
void GPIOs_ISR(void)
{
  ...
  status = M_PSP_READ_REGISTER_32(RGPIO_INTS);
  if (status != 0) {
  ...
  status = M_PSP_READ_REGISTER_32(RGPIO2_INTS);
  if (status != 0) {
    pb = M_PSP_READ_REGISTER_32(GPIO2_BTN);
    pb = pb & 0x1;
    if (pb != 0) {
      if (delay == 0x10000) 
        delay = 0x1000;
...
{
  DisplCount++;
  M_PSP_WRITE_REGISTER_32(GPIO_LEDs, DisplCount);
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0x80);           /* Count reset */
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0x21);           /* Interrupt and count enabled */
...
void GPIO2_Initialization(void)
{
  ...
  M_PSP_WRITE_REGISTER_32(RGPIO2_INTE, 0x1);          /* RGPIO2_INTE */
  M_PSP_WRITE_REGISTER_32(RGPIO2_PTRIG, 0x1);         /* RGPIO2_PTRIG */
  M_PSP_WRITE_REGISTER_32(RGPIO2_INTS, 0x0);          /* RGPIO2_INTS */
  M_PSP_WRITE_REGISTER_32(RGPIO2_CTRL, 0x1);          /* RGPIO2_CTRL */
}

void PTC_Initialization(void)
{
  M_PSP_WRITE_REGISTER_32(RPTC_LRC, delay);         /* Set Timero for 1 sec aprox */
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0x21);           /* Interrupt and count enabled */
}
...
int main(void)
{
  ...
  ExternalIntLine_Initialization(4, 6, GPIOs_ISR);    /* Initialize line IRQ4 with a priority of 6. Set GPIOs_ISR as the Interrupt Service Routine */
  ExternalIntLine_Initialization(3, 7, PTC_ISR);      /* Initialize line IRQ3 with a priority of 7. Set PTC_ISR as the Interrupt Service Routine */
  M_PSP_WRITE_REGISTER_32(Select_INT, 0x3);           /* Connect the GPIO interrupt to the IRQ4 interrupt line */

  /* INITIALIZE THE PERIPHERALS */
  GPIO_Initialization();
  GPIO2_Initialization();
  PTC_Initialization();
